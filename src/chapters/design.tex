...
\section{Syntax}
As introduced in section \ref{OTTR}, OTTR has two different serialisations, wOTTR and stOTTR. As described by Skjæveland et al. stOTTR is made to be a serialisation of OTTR, which is easy for humans two read and write, in addition to being compact\autocite[4]{SLKK_OTTR_2021}. On the other hand, the wOTTR serialisation offers the benefit of leveraging the existing W3C stack, in addition to tools for developing, publishing and maintaining templates\autocite[481-482]{SLKF_OTTR_2018}. For the same reasons, we propose that Frog also have two serialisations, one human-readable syntax that is compact and easy to read and write and one written in RDF for publishing purposes.

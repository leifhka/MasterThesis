...
\begin{figure}
    ...
    \begin{minipage}{.5\textwidth}
      \centering
      \lstinputlisting[language=turtle, firstnumber = 1, basicstyle = \tiny]{examples/design/templateExp.ottr}
    \end{minipage}
    \begin{minipage}{.5\textwidth}
      \centering
      \lstinputlisting[language=turtle, firstnumber = 1, basicstyle = \tiny]{examples/design/resultOfTemplateEx.ttl}
    \end{minipage}
    \label{lol}
    \caption{One the left: an example of have a template generating an fn:plus base function would look. One the right: the result of expanding the instance fn:plus(fn:plusInteger, xsd:integ)}
\end{figure}
...
\para
One approach would have been to generate the functions. As the functions have an RDF syntax, this could have been done by making templates, and using an import statment that imorted  

\para
Ikke enkelt å lage til noen andre en base functions.  
Vanskliger å begrense subtypingen 
